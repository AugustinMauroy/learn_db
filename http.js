...
var read_html   = require('./read_html.js');
...
var server = http.createServer(function(request, response) { 
    ...
    // switch des chemins pour afficher les pages
    switch (path) {  
        case '/':  
        ...
        break;
        case '/post.html':  
            fs.readFile("page/post.html", function(error, data) {  
                if (error) {  
                    response.writeHead(404);  
                    response.write(error);  
                    response.end();  
        ...
        case '/read.html': 
                ...
                console.log(read_html.read_content())
                response.end(); 
        break;
        default:  
        response.writeHead(404, {  
    ...
    //Collecter les données du formulaire POST et les insert dans la db
    if (request.method == 'POST') {
        ...
        request.on('end', function () {
            ...
            con.connect(function(err) {
                if (err) throw err;
                //fonction querry db mysql de post data
                var sqlpost = "INSERT INTO customers.customers (name,email) VALUES ('"+post.name+"','"+post.email+"')";
                console.log(sqlpost);
